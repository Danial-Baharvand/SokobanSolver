...
def test_taboo_cells():
    ...
    '''
        Self made expected answers for test purposes - emily
    '''
    '''
    expected_answer = '  ####   \n' \
                      '###XX####\n' \
    ...
    '''
    '''
    wh.load_warehouse("./warehouses/warehouse_77.txt")
    expected_answer = '########## \n' \
                      '#X      X# \n' \
                      '#X X#### ##\n' \
                      '## #X    X#\n' \
    ...
    '''
    '''
    wh.load_warehouse("./warehouses/warehouse_85.txt")
    expected_answer = '       ####\n' \
                      ' #######XX#\n' \
                      ' #X      X#\n' \
                      ' #       X#\n' \
    ...
    '''
    '''
    wh.load_warehouse("./warehouses/warehouse_163.txt")
    expected_answer = '    #######  \n' \
                      '    #XXXXX## \n' \
                      '##### ###XX##\n' \
                      '#X     X#X ##\n' \
                      ...
                      '        #### '
                      '''
    '''wh.load_warehouse("./warehouses/warehouse_187.txt")
    expected_answer = '                   #####\n' \
                      '                   #XXX#\n' \
                      '           #########  X#\n' \
                      '    ###### #XXX#XXX#  X#\n' \
...
def test_solve_sokoban_elem():
    ...
    answer = solve_sokoban_elem(wh)
    t1 = time.time()
    print('<<  Third test of {} warehouse1>>'.format(fcn.__name__))
    print('Received path: ');
    print(answer)

    print('A* Solver took {:.6f} seconds'.format(t1 - t0))
    # fourth test warehouse 9
    wh = Warehouse()
    ...
    answer = solve_sokoban_elem(wh)
    t1 = time.time()
    print('<<  Fourth test of {} warehouse9>>'.format(fcn.__name__))
    print('Received path: ');print(answer)

    print ('A* Solver took {:.6f} seconds'.format(t1-t0))

    # fifth test warehouse 47
    wh = Warehouse()
    ...
    answer = solve_sokoban_elem(wh)
    t1 = time.time()
    print('<<  Fifth test of {} warehouse47>>'.format(fcn.__name__))
    print('Received path: ');
    print(answer)

    print('A* Solver took {:.6f} seconds'.format(t1 - t0))

    # sixth test warehouse 81
    wh = Warehouse()
    ...
    answer = solve_sokoban_elem(wh)
    t1 = time.time()
    print('<<  Sixth test of {} warehouse81>>'.format(fcn.__name__))
    print('Received path: ');
    print(answer)
